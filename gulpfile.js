...
const  babelcore  =  require('babel-core');  //es6转es5主要模块
...
// }); 
gulp.task('runcss',  function ()  {      return  gulp.src('src/stylesheets/*.css')         .pipe(css())  //调用模块或者包
                 .pipe(gulp.dest('dist/stylesheets/'));  });
gulp.task('uglifyjs',  function ()  {    
    return  gulp.src('src/script/*.js')        .pipe(babel({  //es6转es5
                         presets:  ['es2015']         }))        .pipe(uglifyjs())  //调用模块或者包
...
gulp.task('runimg',  function ()  {    
    return  gulp.src('src/images/*.{png,gif,jpg,ico}')        .pipe(imagemin())        .pipe(gulp.dest('dist/images/'));
});